...
import { Button, Card, Toast, Row, Col } from 'react-bootstrap';
...
function App() {
  ...
  const onSubmit = data =>
    setHomeWork(v => {
      const a = [...v];
      if (a.find(word => word === data['add-home-work'])) {
        setShow(true);
      } else a.push(data['add-home-work']);
      return a;
    });
  ...
  return (
    <>
      <Row>
        <Col xs={12}>
          <Toast onClose={() => setShow(false)} show={show} delay={3000} autohide style={{ backgroundColor: 'red' }}>
            <Toast.Header>
              <strong className="mr-auto">Tarea</strong>
            </Toast.Header>
            <Toast.Body>Tarea ya agregada</Toast.Body>
          </Toast>
        </Col>
      </Row>
      <div className="container-fluid">
